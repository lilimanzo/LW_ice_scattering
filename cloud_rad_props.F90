...
public :: &
   ...
   tamu_ice_get_rad_props_lw,     & ! TAMU ice optics scheme, yihsuan@UMich 2016-05-20
...
! 
   ...
   integer :: i_rei, i_cld ! yihsuan@Umich 2016-05-23
...
subroutine cloud_rad_props_init()
   ...
   i_rei    = pbuf_get_index('REI',errcode=err)  ! yihsuan@UMich 2016-05-23
   i_cld    = pbuf_get_index('CLD',errcode=err)  ! yihsuan@UMich 2016-05-23
...
subroutine tamu_ice_get_rad_props_lw(state, pbuf, ext_od, abs_od, ssa_od, xmomc_od, oldicewp)
...
!--- Local ---!
   ...
   real(r8) :: xmomc_tamu(0:16,nlwbands,pcols,pver) ! ice cloud asymmetric factor (unitless)
...
!  
   ...
   !write(iulog,*) ' LM iciwp: ', iciwp(:,:)
   !write(iulog,*) ' LM rei: ', rei(:,:) 
   !write(iulog,*) ' LM state: ', state%q(:,:,ixcldice)
!--------------------------------------------------------
      ...
      do k = 1,pver
        ...
        if( iciwp(i,k) < 1.e-80_r8 .or. diaice .eq. 0._r8) then
          ...
          if (diaice .ge. 3.0_r8 .and. diaice .le. 370.0_r8) then
             ...
             !write(iulog,*) ' LM invrad: ', invrad ! not zeros; looks fine
             do ib=1,nlwbands
               if (diaice .ge. 25.0_r8) then
                  extcoice(ib) = ((extice4(1,1,ib) * invrad + &
                                   ...
                                   extice4(1,4,ib)
                           !write(iulog,*) ' LM extcoice (diaice>25): ', extcoice(ib) ! zeros
                           !write(iulog,*) ' LM extice4(1,1,ib): ', extice4(1,1,ib) ! zeros
                           !write(iulog,*) ' LM extice4(1,2,ib): ', extice4(1,2,ib) ! zeros
                           !write(iulog,*) ' LM extice4(1,3,ib): ', extice4(1,3,ib) ! zeros
                           !write(iulog,*) ' LM extice4(1,4,ib): ', extice4(1,4,ib) ! zeros
               ...
               !write(iulog,*) ' LM extcoice: ', extcoice(ib)
               !write(iulog,*) ' LM asycoice: ', asycoice(ib)
               !write(iulog,*) ' LM ssacoice: ', ssacoice(ib)
             enddo    ! end do of lwbands for cloud radiative coefficients
             iceind = 2

             do ib=1,nlwbands
                !write(iulog,*) ' LM iciwp: ', iciwp(i,k) ! numbers
                !write(iulog,*) ' LM extcoice: ', extcoice(icb(ib,iceind)) ! zeros
                ...
                write(iulog,*) ' LM taucloud: ', taucloud
                write(iulog,*) ' LM ssacloud: ', ssacloud
                write(iulog,*) ' LM asycloud: ', asycloud
                write(iulog,*) ' LM xmomcloud: ', xmomcloud
                !write(iulog,*) ' LM taucloud: ', taucloud ! zeros
                !write(iulog,*) ' LM ssacloud: ', ssacloud ! zeros
                !write(iulog,*) ' LM xmomc_tamu: ', xmomc_tamu

             enddo    ! end do of lwbands for cloud radiative coefficients
             !write(iulog,*) ' LM taucloud: ', taucloud
             !write(iulog,*) ' LM ssacloud: ', ssacloud
             !write(iulog,*) ' LM asycloud: ', asycloud ! most recent
...
!---------
   ...
   xmomc_od(:,:,:,:) = xmomc_tamu(:,:,:,:)
     ...
     return
end subroutine tamu_ice_get_rad_props_lw
