...
contains
!=========================================================================================
! LM / Yihsuan added ssac, xmomc, ssacmcl, xmomcmcl
subroutine mcica_subcol_lw(lchnk, ncol, nlay, icld, permuteseed, play, &
                       cldfrac, ciwp, clwp, rei, rel, tauc, ssac, xmomc, cldfmcl, &
                       ciwpmcl, clwpmcl, reicmcl, relqmcl, taucmcl, ssacmcl, xmomcmcl)
   ...
   real(kind=r8), intent(in) :: ssac (:,:,:)     ! LM / Yihsuan added Cloud single scattering albedo
                                                 !    Dimensions: (nbndlw,ncol,nlay)
   real(kind=r8), intent(in) :: xmomc(:,:,:,:) ! LM / Yihsuan added Cloud phase function expansion coefficient
   ...
   real(kind=r8), intent(out) :: taucmcl(:,:,:)    ! cloud optical depth [mcica]
                                                     !    Dimensions: (ngptlw,ncol,nlay)
   real(kind=r8), intent(out) :: ssacmcl (:,:,:)   ! LM / Yihsuan added Cloud single scattering albedo [mcica]
                                                     !    Dimensions: (ngptlw,ncol,nlay)
   real(kind=r8), intent(out) :: xmomcmcl (:,:,:,:) ! LM / Yihsuan added Cloud phase function expansion coefficient [mcica]
   ...
   call generate_stochastic_clouds( &
      ...
      cldfrac, clwp, ciwp, tauc, ssac, xmomc, cldfmcl, &
      clwpmcl, ciwpmcl, taucmcl, ssacmcl, xmomcmcl, permuteseed)

end subroutine mcica_subcol_lw

!=========================================================================================
! LM / Yihsuan added ssac, xmomc, ssac_stoch, xmomc_stoch
subroutine generate_stochastic_clouds( &
   ncol, nlay, nsubcol, icld, pmid, &
   cld, clwp, ciwp, tauc, ssac, xmomc, cld_stoch, &
   clwp_stoch, ciwp_stoch, tauc_stoch, ssac_stoch, xmomc_stoch, changeSeed) 
   ...
   real(kind=r8), intent(out) :: ssac_stoch (:,:,:)   ! LM / Yihsuan added Cloud single scattering albedo [mcica]
                                                         !    Dimensions: (ngptlw,ncol,nlay)
   real(kind=r8), intent(out) :: xmomc_stoch (:,:,:,:) ! LM / Yihsuan added Cloud phase function expansion coefficient [mcica]
                                                         !    Dimensions: (moment,ngptlw,ncol,nlay)                                                  
   ! -- Local variables 
