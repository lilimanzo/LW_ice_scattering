...
use rrtmg_lw_rtrnmc,     only: rtrnmc, rtr2function ! LM / Yihsuan added rtr2function
use cam_logfile,         only: iulog ! LM added 
implicit none
...
contains
!=========================================================================================
! LM / Yihsuan add flag_rtr2, ssacmcl, xmomcmcl
subroutine rrtmg_lw &
            (lchnk   ,ncol    ,nlay    ,icld    ,flag_rtr2,          &
             ...
             cldfmcl ,taucmcl ,ssacmcl ,xmomcmcl,ciwpmcl ,clwpmcl , &
             reicmcl ,relqmcl , &
             tauaer  , &
...
!     Apr 2008: M. J. Iacono, AER, Inc.
   ...
   ! LM / Yihsuan added flag_rtr2, ssacmcl, xmomcmcl
   ...
   ! LM / Yihsuan added ssacmc, xmomcmc
   ...
   do iplon = 1, ncol
      ...
      ! LM / Yihsuan added ssacmcl, xmomcmcl, ssacmc, xmomcmc
      call inatm(iplon, nlay, icld, iaer, &
                 play, plev, tlay, tlev, tsfc, h2ovmr, &
                 o3vmr, co2vmr, ch4vmr, o2vmr, n2ovmr, cfc11vmr, cfc12vmr, &
                 cfc22vmr, ccl4vmr, emis,  &
                 cldfmcl, taucmcl, ssacmcl, xmomcmcl, ciwpmcl, clwpmcl, reicmcl, relqmcl, tauaer, &
                 pavel, pz, tavel, tz, tbound, semiss, coldry, &
                 wkl, wbrodl, wx, pwvcm, &
                 cldfmc, taucmc, ssacmc, xmomcmc, ciwpmc, clwpmc, reicmc, dgesmc, relqmc, taua)
...
subroutine inatm(iplon, nlay, icld, iaer, &
   ...
   real(kind=r8), intent(in) :: tauaer(:,:,:)        ! Aerosol optical depth
                                                     !    Dimensions: (ncol,nlay,nbndlw)
   ! LM / Yihsuan added ssacmcl, xmomcmcl
   real(kind=r8), intent(in) :: ssacmcl(:,:,:)      ! Cloud single scattering albedo
                                                        !    Dimensions: (ngptlw,ncol,nlay)
   real(kind=r8), intent(in) :: xmomcmcl(0:,:,:,:)    ! cloud phase  function expansion coefficient
                                                        !    Dimensions: (moments,ngptlw,ncol,nlay)
   ...
   if (icld >= 1) then 
      ...
      do l = 1, nlay-1
         do ig = 1, ngptlw
            ...
            ssacmc (ig,l)   = ssacmcl (ig,iplon,nlay-l) ! LM / Yihsuan added
            xmomcmc(:,ig,l) = xmomcmcl(:,ig,iplon,nlay-l) ! LM / Yihsuan added
         end do
