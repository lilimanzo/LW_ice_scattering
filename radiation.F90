...
subroutine radiation_tend( &
   ...
   use cloud_rad_props,    only: get_ice_optics_sw, get_liquid_optics_sw, liquid_cloud_get_rad_props_lw, &
                                 ...
                                 grau_cloud_get_rad_props_lw, get_grau_optics_sw, & ! LM commented
                                 snow_cloud_get_rad_props_lw, get_snow_optics_sw, & 
                                 tamu_ice_get_rad_props_lw ! LM added
   ...
   use parrrtm,            only: nbndlw ! LM added
   ...
   !--------------------------------------------------------------------------------------
   ! LM added Yihsuan modifications
    ...
    logical, parameter :: mc6_scat_flag  = .True.  ! flag to turn on/off MC6 (MODIS Collection 6) ice optical parameterization with scattering
                                               ! .True.   : turn on  ice cloud LW scattering
                                               ! .False.  : turn off ice cloud LW scattering
   ...
   ! LM end Yihsuan modifications

   ! LM more Yihsuan modifications
   ice_lw_ext   = 0._r8
   ice_lw_ssa   = 0._r8
   ice_lw_xmomc = 0._r8
   ...
   if (dosw .or. dolw) then
      ...
      ! LM added Yihsuan modifications
      if (.not.mc6_flag) then  ! use standard ice optics scheme with either rtr2 or rtrnmc
         ...
         if (icecldoptics .eq. "ebertcurry") then
           call tamu_ice_get_rad_props_lw(state, pbuf, ice_lw_ext, ice_lw_abs_mc6, ice_lw_ssa, ice_lw_xmomc, oldicewp=.true.)
         else
           call tamu_ice_get_rad_props_lw(state, pbuf, ice_lw_ext, ice_lw_abs_mc6, ice_lw_ssa, ice_lw_xmomc, oldicewp=.false.)
         end if

         !write(iulog,*) 'LM ice_lw_ext = ', ice_lw_ext(:,1:ncol,:)   ! LM commented
         !write(iulog,*) 'LM ice_lw_abs_mc6 = ', ice_lw_abs_mc6(:,1:ncol,:)   ! LM commented
         !write(iulog,*) 'LM ice_lw_ssa = ', ice_lw_ssa(:,1:ncol,:)   ! LM commented
         !write(iulog,*) 'LM ice_lw_xmomc = ', ice_lw_xmomc(:,:,:,:)   ! LM commented
         ...
         do i=1,ncol
           do k=1,pver
              if(cldfprime(i,k) > 0. .and. cldfsnow_idx > 0)then
              ...
              else if (cldfprime(i,k) > 0. .and. .not.(cldfsnow_idx > 0))then
                 cld_lw_abs(1:nbndlw,i,k)= cld_lw_abs_mc6(1:nbndlw,i,k)
                 cld_lw_ext(1:nbndlw,i,k)= cld_lw_ext    (1:nbndlw,i,k)
         ...
         do jband = 1,nbndlw
           ...
           if (cldfprime(jcol,jlev) > 0. .and. cld_lw_ext(jband,jcol,jlev) .gt. 0._r8) then ! LM changed
             ...
             !write(iulog,*) 'LM ice_lw_ssa = ', ice_lw_ssa(jband,jcol,jlev) ! now values!
             !write(iulog,*) 'LM ice_lw_ext = ', ice_lw_ext(jband,jcol,jlev) ! now values!
             !write(iulog,*) 'LM cld_lw_ext = ', cld_lw_ext(jband,jcol,jlev) ! values
           else
         ...
         j=1
         do jcol  = 1,ncol
           ! write(iulog,*) 'LM XMOMC LOOP REACHED, jcol=', jcol
         do jlev  = 1,pver
         do jband = 1,nbndlw
           !write(iulog,*) 'LM cldfprime = ', cldfprime(jcol,jlev)
           !write(iulog,*)  'LM cld_lw_ext =  ', cld_lw_ext(jband,jcol,jlev)
           !write(iulog,*)  'LM cld_lw_ssa = ', cld_lw_ssa(jband,jcol,jlev)
           !if (cldfprime(i,k) > 0. .and. cld_lw_ext(jband,jcol,jlev) .gt. 0._r8 .and. cld_lw_ssa(jband,jcol,jlev) .gt. 0._r8 ) then
           if (cldfprime(jcol,jlev) > 0. .and. cld_lw_ext(jband,jcol,jlev) .gt. 0._r8 .and. cld_lw_ssa(jband,jcol,jlev) .gt. 0._r8 ) then ! LM changed
             ...
             !write(iulog,*) 'LM DEBUG ice_lw_xmomc = ', ice_lw_xmomc(j,jband,jcol,jlev) ! not zeros anymore!
             !write(iulog,*) 'LM DEBUG cld_lw_xmomc = ', cld_lw_xmomc(j,jband,jcol,jlev) ! not zeros anymore!
           else
         ...
         ! LM DEBUG: compare cld_lw_xmomc against ice_lw_xmomc
         !do jcol = 1,ncol
         !  do jlev = 1,pver
         !    if (cld_lw_ssa(1,jcol,jlev) > 0._r8) then
         !      write(iulog,*) 'LM DEBUG xmomc jcol=',jcol,' jlev=',jlev, &
         !           ' cld_lw_xmomc(1)=', cld_lw_xmomc(1,1,jcol,jlev), &
         !           ' ice_lw_xmomc(1)=', ice_lw_xmomc(1,1,jcol,jlev), &
         !           ' cld_lw_ssa=', cld_lw_ssa(1,jcol,jlev)
         !    endif
         !  enddo
         !enddo

         ! return variable
         if (rtr2_flag .and. mc6_scat_flag) then   ! use rtr2 with MC6 absorption+scattering
         ...
         do icall = N_DIAG, 0, -1
            ...
            if (active_calls(icall)) then
               ...
               call rad_rrtmg_lw( &
                  ...
                  lu, ld,                                            &
                  rtr2_flag, c_cld_lw_ssa, c_cld_lw_xmomc,           & ! LM added
                  surface_emis, ful, fsul, fdl, fsdl, lwdn_spec)       ! LM added 
